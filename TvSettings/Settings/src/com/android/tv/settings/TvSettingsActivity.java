...
public abstract class TvSettingsActivity extends Activity {
  ...
  public void finish() {
    final Fragment fragment =
        getFragmentManager().findFragmentByTag(SETTINGS_FRAGMENT_TAG);
    if (isResumed() && fragment != null) {
      ...
      scene.setEnterAction(()
                               -> getFragmentManager()
                                      ...
                                      .commitNow());
      final Slide slide = new Slide(Gravity.END);
      slide.setSlideFraction(getResources().getDimension(
          root.getWidth())); // getResources().getDimension(R.dimen.lb_settings_pane_width)
                             // / root.getWidth());
