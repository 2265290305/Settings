...
public class AutoBlueToothManager {
    ...
    public void start() {
        ...
        registerReceivers();
        mBtAdapter.getProfileProxy(mContext, mListener, HID_HOST_PROFILE);
    ...
    private void closeProfileProxy() {
        if (mBtAdapter != null && mService != null) {
            try {
                mBtAdapter.closeProfileProxy(HID_HOST_PROFILE, mService);
            } catch (Exception e) {
    ...
    public BluetoothDevice findRemoteByAddress(String address) {
        if (mBtAdapter == null) return null;
        Set<BluetoothDevice> pairedDevices = mBtAdapter.getBondedDevices();
        for (BluetoothDevice device : pairedDevices) {
            if (judgeRC(device) && (address == null || address.equals(device.getAddress()))) {
                return device;
            }
        }
    ...
    public void disconnectRemote(String address) {
        BluetoothDevice device = findRemoteByAddress(address);
        if (device == null) {
            Log.w(TAG, "disconnectRemote remote not found address=" + address);
            return;
        }
        disconnectDevice(device);
    }

    public void forgetRemote(String address) {
        BluetoothDevice device = findRemoteByAddress(address);
        if (device == null) {
            Log.w(TAG, "forgetRemote remote not found address=" + address);
            return;
        }
        // “忽略此设备”与系统蓝牙设置里的“忘记此设备”一致：删除系统配对记录。
        unpairDevice(device);
        if (mCallback != null) {
            mCallback.onBondRemoved(device);
        }
    }
    ...
    public void disconnectDevice(BluetoothDevice device) {
        Log.i(TAG, "[disconnectDevice]断开设备: " + device.getName());
        if (mBtAdapter != null) {
            mBtAdapter.cancelDiscovery();
        }
        if (mService != null) {
            try {
                Method method = mService.getClass().getMethod("disconnect", BluetoothDevice.class);
