...
private const val PRIVACY_CAMERA_FORBID_KEY = "cameraForbid"
private const val PRIVACY_CAMERA_NOT_FORBIDDEN = "1"
private const val PRIVACY_CAMERA_FORBIDDEN = "2"
...
private val PRIVACY_CAMERA_CONTROL_URIS: List<Uri> = listOf(
    Uri.parse("content://com.android.zshd.deviceinfo/cameraStat")
)
private val PRIVACY_VOICE_PROVIDER_URI: Uri = Uri.parse("content://com.android.ctcc.voice/settings")
private val PRIVACY_CAMERA_NOTIFY_URIS: List<Uri> = listOf(
    Uri.parse("content://com.android.zshd.deviceinfo/cameraStat"),
    Uri.parse("content://com.android.ctcc.deviceinfo/cameraStat")
)
// provider notifyChange 会发到 devStat 子 URI（如 .../devStat?micMute=1），这里监听根节点并允许子节点回调。
...
private fun queryProviderValue(context: Context, uris: List<Uri>, key: String, defaultValue: String): String {
    ...
    return uris.firstNotNullOfOrNull { uri ->
        runCatching {
            ...
            Log.d(PRIVACY_VOICE_TAG, "provider query uri=$uri key=$key result=${dumpPrivacyBundle(result)}")
...
private fun updateProviderValue(context: Context, uris: List<Uri>, key: String, value: String): Boolean {
    val resolver = context.contentResolver
    return uris.any { uri ->
        runCatching {
            val extras = Bundle().apply {
            ...
            Log.d(PRIVACY_VOICE_TAG, "provider opt uri=$uri key=$key value=$value result=${dumpPrivacyBundle(result)}")
...
private fun queryCameraForbid(context: Context): String {
    return queryProviderValue(
        context = context,
        uris = PRIVACY_CAMERA_CONTROL_URIS,
        key = PRIVACY_CAMERA_FORBID_KEY,
        defaultValue = PRIVACY_CAMERA_NOT_FORBIDDEN
    )
}

private fun updateCameraForbid(context: Context, value: String): Boolean {
    val resolver = context.contentResolver
    return PRIVACY_CAMERA_CONTROL_URIS.any { uri ->
        runCatching {
            val targetUri = uri.buildUpon()
                .appendQueryParameter(PRIVACY_CAMERA_FORBID_KEY, value)
                .build()
            val extras = Bundle().apply {
                putString(PRIVACY_CAMERA_FORBID_KEY, value)
            }
            val result = resolver.call(targetUri, PRIVACY_METHOD_DEV_OPT, null, extras)
            Log.d(PRIVACY_VOICE_TAG, "camera opt uri=$targetUri value=$value result=${dumpPrivacyBundle(result)}")
            result == null
                || isPrivacyBundleSuccess(result)
                || privacyNormalizeValue(result.getString(PRIVACY_CAMERA_FORBID_KEY)) == value
                || privacyNormalizeValue(result.getString("value")) == value
                || privacyNormalizeValue(result.getString("result"))?.equals("true", ignoreCase = true) == true
        }.getOrDefault(false)
    }
...
private fun queryPrivacyValue(context: Context, key: String, defaultValue: String): String {
    return queryProviderValue(context, PRIVACY_MIC_CONTROL_URIS, key, defaultValue)
}

private fun updatePrivacyValue(context: Context, key: String, value: String): Boolean {
    return updateProviderValue(context, PRIVACY_MIC_CONTROL_URIS, key, value)
}
...
private fun isCameraEnabled(context: Context): Boolean {
    return when (queryCameraForbid(context)) {
        PRIVACY_CAMERA_FORBIDDEN -> false
        PRIVACY_CAMERA_NOT_FORBIDDEN -> true
        else -> true
    }
}

private fun updateCameraEnabled(context: Context, enabled: Boolean): Boolean {
    val value = if (enabled) PRIVACY_CAMERA_NOT_FORBIDDEN else PRIVACY_CAMERA_FORBIDDEN
    val optSent = updateCameraForbid(context, value)
    val queriedValue = queryCameraForbid(context)
    val updated = queriedValue == value
    Log.d(PRIVACY_VOICE_TAG, "camera update target=$value optSent=$optSent queried=$queriedValue updated=$updated")
    Toast.makeText(
        context,
        if (updated) "摄像头已${if (enabled) "启用" else "禁用"}" else "操作失败",
        Toast.LENGTH_SHORT
    ).show()
    return updated
}
...
fun PrivacyScreen(modifier: Modifier = Modifier) {
    ...
    var voiceRefreshVersion by remember { mutableStateOf(0) }
    var cameraRefreshVersion by remember { mutableStateOf(0) }

    DisposableEffect(context) {
        val observer = object : ContentObserver(Handler(Looper.getMainLooper())) {
            override fun onChange(selfChange: Boolean, uri: Uri?) {
                when (uri?.path) {
                    "/cameraStat" -> cameraRefreshVersion++
                    "/devStat" -> voiceRefreshVersion++
                    else -> {
                        voiceRefreshVersion++
                        cameraRefreshVersion++
                    }
                }
            }
        }
        // 监听 micMute 与 cameraForbid 对应的 notify 根节点；provider 会在子 URI 上带查询参数。
        PRIVACY_MIC_MUTE_NOTIFY_URIS.forEach { uri ->
            runCatching { context.contentResolver.registerContentObserver(uri, true, observer) }
        }
        PRIVACY_CAMERA_NOTIFY_URIS.forEach { uri ->
            runCatching { context.contentResolver.registerContentObserver(uri, true, observer) }
        }
        onDispose {
    ...
    LaunchedEffect(context, voiceRefreshVersion) {
        voiceWake = queryVoiceWakeEnabled(context)
    }

    LaunchedEffect(context, cameraRefreshVersion) {
        ...
        Card(
            ...
            Column(
                ...
                ToggleItem(
                    ...
                    onCheckedChange = {
                        updateCameraEnabled(context, it)
                        cameraEnabled = isCameraEnabled(context)
                    }
                )
            }
